...
public class ClasExtProd extends javax.swing.JFrame 
{
    ...
    {                
        ...
        this.setTitle("Catálogo de clasificación extra, Usuario: <" + Login.usuario + "> " + Login.sFLog);        
            ...
            public void propertyChange(PropertyChangeEvent event) 
            {
                ...
                if("tableCellEditor".equals(pro)) 
                {
                    ...
                    else
                    {
                        ...
                        try 
                        {                                                        
                            sQ = "UPDATE clasprov SET descrip = '" + sDescrip.replace("'", "''").replace(",", "") + "', fmod = now(), estac = '" + Login.usuario.replace("'", "''") + "', sucu = '" + Star.sSucu.replace("'", "''") + "', nocaj = '" + Star.sNoCaj.replace("'", "''") + "' WHERE cod = '" + sCodOriG.replace("'", "''") + "' AND idempresa = '" + Login.codigo_empresa + "'";                                                
    ...
    {        
        ...
        try
        {
            sQ = "SELECT cod, descrip FROM clasprod WHERE idempresa = '" + Login.codigo_empresa + "'";
            st = con.createStatement();
    ...
    private void jBDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDelActionPerformed
        ...
        for(int x = iSel.length - 1; x >= 0; x--)
        {
            ...
            try
            {
                sQ = "SELECT codext FROM prods WHERE codext = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND idempresa = '" + Login.codigo_empresa + "'";                        
                st = con.createStatement();
                rs = st.executeQuery(sQ);
            ...
            try 
            {                
                sQ = "DELETE FROM clasprod WHERE cod = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND idempresa = '" + Login.codigo_empresa + "'";                                            
                st = con.createStatement();
                st.executeUpdate(sQ);
             }
             catch(SQLException expnSQL) 
            ...
            try 
            {            
                sQ = "INSERT INTO logclasext(cod,                                                                                           descrip,                                                                                    accio,               estac,                      sucu,                     nocaj,                falt,       idempresa) " + 
                                "VALUES('" + jTab.getValueAt(iSel[x], 1).toString().replace("'", "''").replace(",", "") + "', '" +          jTab.getValueAt(iSel[x], 2).toString().replace("'", "''").replace(",", "") + "',            'BORRAR',     '" +   Login.usuario + "',   '" +    Star.sSucu + "',  '" +    Star.sNoCaj + "',     now(), '" + Login.codigo_empresa + "')";                                
                st = con.createStatement();
                st.executeUpdate(sQ);
            }
            catch(SQLException expnSQL) 
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        try 
        {            
            sQ = "INSERT INTO clasprod(     cod,                                                                    descrip,                                        falt,       estac,                                      sucu,                                     nocaj,                                    idempresa) " + 
        ...
        {            
            sQ = "INSERT INTO logclasext(cod,                                                                       descrip,                                            accio,                  estac,                      sucu,                     nocaj,                falt,       idempresa) " + 
                            "VALUES('" + jTCod.getText().replace(" ", "").trim().replace("'", "''") + "', '" +      jTDescrip.getText().replace("'", "''") + "',        'AGREGAR',      '" +    Login.usuario + "',   '" +    Star.sSucu + "',  '" +    Star.sNoCaj + "',     now(), '" + Login.codigo_empresa + "')";                                
            st = con.createStatement();
            st.executeUpdate(sQ);
        }
        catch(SQLException expnSQL) 
    ...
    private void jBBuscActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscActionPerformed
        ...
        try
        {                  
            sQ = "SELECT * FROM clasprod WHERE cod LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR descrip LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR falt LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR fmod LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') AND idempresa = '" + Login.codigo_empresa + "'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
